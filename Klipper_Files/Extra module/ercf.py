...
class Ercf:
    ...
    def __init__(self, config):
        ...
        self.encoder_resolution = config.getfloat("encoder_resolution", 1.5, above=0.0)
        self.encoder_sample_time = config.getfloat(
            "encoder_sample_time", 0.1, above=0.0
        )
        self.encoder_poll_time = config.getfloat("encoder_poll_time", 0.0001, above=0.0)
        ...
        # Parameters
        self.long_moves_speed = config.getfloat("long_moves_speed", 100.0)
        self.long_moves_accel = config.getfloat("long_moves_accel", 400.0)
        self.short_moves_speed = config.getfloat("short_moves_speed", 25.0)
        self.short_moves_accel = config.getfloat("short_moves_accel", 400.0)
        # GCODE commands
        ...
        self.gcode.register_command(
            "ERCF_LOAD", self.cmd_ERCF_LOAD, desc=self.cmd_ERCF_LOAD_help
        )
        self.gcode.register_command(
            "ERCF_UNLOAD", self.cmd_ERCF_UNLOAD, desc=self.cmd_ERCF_UNLOAD_help
        )
        self.gcode.register_command(
        ...
        self.gcode.register_command(
            "ERCF_SET_STEPS", self.cmd_ERCF_SET_STEPS, desc=self.cmd_ERCF_SET_STEPS_help
        )
        self.gcode.register_command(
    ...
    def _gear_stepper_move_wait(self, dist, wait=True, speed=None, accel=None):
        ...
        if accel is None:
            accel = self.long_moves_accel if is_long_move else self.short_moves_accel
        self.gear_stepper.do_move(dist, speed, accel, True)
        if wait:
            self.toolhead.wait_moves()

    def _selector_stepper_move_wait(
        self, dist, home=0, wait=True, speed=80.0, accel=1800
    ):
        homing_string = ""
        wait_string = ""
        if home != 0:
        ...
        if not wait:
            wait_string = " SYNC=0"
        command_string = (
            "MANUAL_STEPPER STEPPER=selector_stepper"
    ...
    def cmd_ERCF_CALIBRATE_ENCODER(self, gcmd):
        dist = gcmd.get_float("DIST", 500.0, above=0.0)
        repeats = gcmd.get_int("RANGE", 5, minval=1)
        speed = gcmd.get_float("SPEED", self.long_moves_speed, above=0.0)
        accel = gcmd.get_float("ACCEL", self.long_moves_accel, above=0.0)
        ...
        gcmd.respond_info(
            "Load direction: mean=%(mean).2f stdev=%(stdev).2f"
            " min=%(min)d max=%(max)d range=%(range)d" % self._sample_stats(plus_values)
        )
        gcmd.respond_info(
            "Unload direction: mean=%(mean).2f stdev=%(stdev).2f"
            " min=%(min)d max=%(max)d range=%(range)d" % self._sample_stats(min_values)
        )
        ...
        if half_mean == 0:
            gcmd.respond_info(
                "No counts measured. Ensure a tool was selected "
        ...
        gcmd.respond_info("Before calibration measured length = %.6f" % old_result)
        gcmd.respond_info("Resulting resolution for the encoder = %.6f" % resolution)
        gcmd.respond_info("After calibration measured length = %.6f" % new_result)

    cmd_ERCF_HOME_EXTRUDER_help = "Home the filament tip on the toolhead sensor"

    def cmd_ERCF_HOME_EXTRUDER(self, gcmd):
        homing_length = gcmd.get_float("TOTAL_LENGTH", 100.0, above=0.0)
        step_length = gcmd.get_float("STEP_LENGTH", 1.0, above=0.0)
        both_in_sync = True
        homing_speed = 25.0
        sensor = self.printer.lookup_object("filament_switch_sensor toolhead_sensor")
            ...
            if bool(sensor.runout_helper.filament_present) == sensor_state:
                ...
                if both_in_sync:
                    ...
                    self.gear_stepper.do_move(
                        step_length, homing_speed, self.short_moves_accel, False
                    )
    ...
    def cmd_ERCF_LOAD(self, gcmd):
        ...
        if self._counter.get_distance() <= 6.0:
            self.gcode.respond_info(
                "Error loading filament to ERCF, no"
            ...
            if self._counter.get_distance() <= 6.0:
                self.gcode.respond_info(
                    "Still no filament loaded in the ERCF,"
                    " calling %s..." % self.MACRO_PAUSE
                )
                self.gcode.run_script_from_command(self.MACRO_UNSELECT_TOOL)
                self.gcode.run_script_from_command(self.MACRO_PAUSE)
                return
        ...
        if req_length != 0:
            ...
            for i in range(num_moves):
                self._gear_stepper_move_wait(
                    (req_length / num_moves) - (counter_distance / num_moves)
                )
            counter_distance = self._counter.get_distance()
            ...
            if diff_distance <= 6.0 or not iterate:
            ...
            for retry_attempts in range(2):
                ...
                counter_distance = self._counter.get_distance()
                self.gcode.respond_info(
                    "Correction load move done,"
                    " requested = %.1f, measured = %.1f"
                    % (req_length, counter_distance)
                )
                diff_distance = req_length - counter_distance
                if diff_distance <= 6.0:
    ...
    def cmd_ERCF_UNLOAD(self, gcmd):
        ...
        req_length = gcmd.get_float("LENGTH", 1200.0)
        ...
        if homing_move:
            iterate = False
            for step in range(int(req_length / 15.0)):
                self._counter.reset_counts()
                self._gear_stepper_move_wait(-15.0)
                delta = 15.0 - self._counter.get_distance()
                # Filament is now out of the encoder
                if delta >= 3.0:
                    self._counter.reset_counts()
                    self._gear_stepper_move_wait(-(23.0 - delta))
        ...
        if iterate:
            ...
            if delta_length >= 3.0:
                ...
                counter_distance = self._counter.get_distance()
                self.gcode.respond_info(
                    "Correction unload move done,"
                    " requested = %.1f, measured = %.1f"
                    % (req_length, counter_distance)
                )
                if (req_length - counter_distance) >= 15.0:
                    # Unload failed
                    self.gcode.respond_info(
            ...
            for step in range(int(buffer_length / 15.0) + 2):
                self._counter.reset_counts()
                self._gear_stepper_move_wait(-15.0)
                delta = 15.0 - self._counter.get_distance()
                # Filament is now out of the encoder
            ...
            self.gcode.respond_info(
                ...
                " please check the ERCF, calling %s..." % self.MACRO_PAUSE
            )
            self.gcode.run_script_from_command(self.MACRO_UNSELECT_TOOL)
            self.gcode.run_script_from_command(self.MACRO_PAUSE)
    ...
    def cmd_ERCF_SET_STEPS(self, gcmd):
        ratio = gcmd.get_float("RATIO", 1.0, above=0.0)
    ...
    def cmd_ERCF_GET_SELECTOR_POS(self, gcmd):
        ref_pos = gcmd.get_float("REF", 0.0)
        ...
        traveled_position = (
            abs(current_position - init_position)
            * self.selector_stepper.steppers[0].get_step_dist()
        )
        self.gcode.respond_info("Selector position = %.1f " % (traveled_position))
    ...
    def cmd_ERCF_MOVE_SELECTOR(self, gcmd):
        ...
        if abs(travel) <= 2.3:
            ...
            self.gcode.respond_info(
                "Selector is blocked by inside filament," " trying to recover..."
            )
            # Realign selector
            self.selector_stepper.do_set_position(0.0)
    ...
    def cmd_ERCF_FINALIZE_LOAD(self, gcmd):
        length = gcmd.get_float("LENGTH", 30.0, above=0.0)
        tune = gcmd.get_int("TUNE", 0)
        threshold = gcmd.get_float("THRESHOLD", 10.0, above=0.0)
        if length is None:
        ...
        if tune == 1:
            self.gcode.respond_info(
                "Measured value from the encoder was %.1f" % final_encoder_pos
            )
        ...
        if final_encoder_pos < threshold:
            self.gcode.respond_info(
                ...
                " calling %s..." % (threshold, final_encoder_pos, self.MACRO_PAUSE)
            )
            self.gcode.run_script_from_command(self.MACRO_PAUSE)
            return
